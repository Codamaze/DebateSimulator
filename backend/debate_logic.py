...
class DebateState:
    def __init__(self, resolution: str, user_role: str, ai_difficulty: str, mode: str = "user-vs-ai"):
        ...
        self.user_role = user_role.lower()  # "pro" or "con"
    ...
    def advance_phase(self):
        """
        Advances to the next structured debate phase (e.g., from Constructive to Rebuttal),
        not used for back-and-forth Crossfire turns.
        """
    ...
    def _set_speaker_by_phase(self):
        """
        Sets speaker based on the current phase and user/AI role.
        Alternates between speakers in Crossfire phases.
        """
        if self.current_phase in [
            ...
            # Alternate between user and ai during Crossfire
            self.current_speaker = "user" if self.current_speaker == "ai" else "ai"
        elif self.current_phase == DebatePhase.JUDGING_PHASE:
            self.current_speaker = "ai"

    def is_crossfire_phase(self):
        """
        Returns True if the current phase is a crossfire (Q&A) round.
        """
